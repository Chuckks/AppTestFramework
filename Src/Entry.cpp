...
int keyboard_event( NO_SEGMENT noappli, S_KEY* key_in, S_KEY* key_out )
{
	...
	switch( key_in->keycode )
	{
		case N0:
		{
			...
			//bufferTest.Read( );
			//bufferTest.SetChar( );
			//bufferTest.SearchChar( );
			//bufferTest.Write( );
			//bufferTest.SubBuffer();
			bufferTest.StaticMethods( );
			key_out->keycode = 0;
			break;
		}
		case N1:
...
int time_function( NO_SEGMENT no, void* p1, void* p2 )
{
	...
	GL_Dialog_Message( gGoalGraphicLibInstance, appName, tcDisplay, GL_ICON_NONE, GL_BUTTON_NONE, 2 * GL_TIME_SECOND );
